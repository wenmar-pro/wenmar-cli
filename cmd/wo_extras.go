...
// wo_extras.go holds the work-order show command and its five tab
...
// else under "wo" is generated (gen_wo.go).
...
func runWorkOrdersShow(cmd *cobra.Command, args []string) error {
	...
	opts := output.Options{Mode: mode, JQFilter: jqFlag, Breadcrumbs: showBreadcrumbs("wo", args[0]), Notice: notice}
...
func runWorkOrdersTab(tab string) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		return runShow(cmd, args, "wo", "GET", func(a []string) string { return fmt.Sprintf("/work_orders/%s/%s", a[0], tab) }, func(ctx context.Context, client *wenmar.Client, id int) (any, error) {
